...
const createMenu = (objeto) => {
  const object = {
    fetchMenu: () => objeto,
    consumption: [],
    order: (string) => object.consumption.push(string),
    pay: () => {
      let somaDosPreçosDosPedidos = 0;
      const cardapinho = { ...cardapio.food, ...cardapio.drinks };
      for (let prices of object.consumption) {
        somaDosPreçosDosPedidos += cardapio[prices];
      }
      return somaDosPreçosDosPedidos * 1.1;
    },
  };
  return object;
};
