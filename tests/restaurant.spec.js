...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    expect(createMenu({ drink: 'water' }).fetchMenu()).toEqual({ drink: 'water' });
    expect(typeof createMenu({ drink: 'water' }).fetchMenu).toEqual('function');
    ...
    expect(objetoRetornado.consumption).toEqual(['cerveja']);

    // TESTE 6: Verifique se, ao adicionar três pedidos, dentre bebidas e comidas, o array `objetoRetornado.consumption` contém os itens pedidos.
    objetoRetornado.order('agua');
    objetoRetornado.order('sopa');
    expect(objetoRetornado.consumption).toEqual(['cerveja', 'agua', 'sopa']);
    ...
    expect(objetoRetornado.consumption).toEqual([ 'cerveja', 'agua', 'sopa', 'cerveja', 'cerveja', 'cerveja' ]);
    ...
    expect(objetoRetornado.pay()).toEqual(NaN);
    // Ir na mentoria e tirar dúvida sobre essa questão
